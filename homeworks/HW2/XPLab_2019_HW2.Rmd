...
R version 3.6.0 running on x86_64-pc-linux-gnu (64-bit)
...
filter(tooth_growth_tibble, supp=="OJ")
...
filter(tooth_growth_tibble, supp=="VC", dose>=1.0)
...
tooth_growth_tibble %>% group_by(supp) %>%  summarise(mean=mean(len)) 
...
ggplot(data=tooth_growth_tibble) + 
...
*ANSWER:*
Only by visually inspecting the graph, I'd say there may be a difference between both types of supplement, where those guinea pigs that received OJ tend to show greater tooth lengths - at least for doses of 0.5 and 1.0 mg/day, for a larger dosis of 2mg/day, the difference, if any, seems to be smaller.
